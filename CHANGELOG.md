...
-   `AddressBookQueryWeb` now bounds every mirror node request with a 30 s timeout by default. `WebClient` and `NativeClient` never passed one, so a mirror node that accepted the connection and never answered hung `updateNetwork()`, `forMainnetAsync()` and `forTestnetAsync()` indefinitely. The timeout applies per attempt and per page, and an explicit `execute(client, requestTimeout)` still overrides it. Runtimes without `AbortSignal.timeout` fall back to `AbortController`. [#4378](https://github.com/hiero-ledger/hiero-sdk-js/issues/4378)
### Fixed
