    ...
    async _makeFetchRequest(client, resolve, reject, requestTimeout) {
        ...
        try {
            ...
            this._addresses = nodes.map((node) => {
                ...
                // Convert base64 string to Uint8Array using browser's atob
                const certHash = new Uint8Array(
                    atob(node.node_cert_hash)
                        .split("")
                ...
                return new NodeAddress({
                    ...
                    addresses: node.service_endpoints.map(
                        (endpoint) =>
                            new EndPoint({
                                address: endpoint.ip_address_v4,
                                port: endpoint.port,
                            }),
                    ),
            ...
            resolve(new NodeAddressBook({ nodeAddresses: this._addresses }));
        } catch (error) {
            const message =
