...
async function main() {
    ...
    await (
        await batchTx
            .addTransaction(accountCreateTx1)
            .addTransaction(accountCreateTx2)
            .addTransaction(accountCreateTx3)
            .execute(client)
    ).getReceipt(client);
    ...
    const transactionIds = batchTx.getTransactionIds();
    ...
    const receipt2 = await new TransactionReceiptQuery()
        .setTransactionId(transactionIds[1])
        .execute(client);
    const receipt3 = await new TransactionReceiptQuery()
        .setTransactionId(transactionIds[2])
        .execute(client);
