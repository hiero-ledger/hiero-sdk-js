...
describe.only("BatchTransaction", function () {
    ...
    describe("constructor", function () {
        it("should initialize with empty transactions array when no options provided", function () {
            expect(batchTransaction.transactions).to.deep.equal([]);
        });
        ...
        it("should initialize with provided transactions", function () {
            ...
            expect(batchTransaction.transactions).to.be.equal(transactions);
        });
    });
    ...
    describe("setTransactions", function () {
        it("should set transactions and return this for chaining", function () {
            const transactions = [mockTransaction1, mockTransaction2];
            const result = batchTransaction.setTransactions(transactions);

            expect(batchTransaction.transactions).to.be.eql(transactions);
    ...
    describe("addTransaction", function () {
        it("should add a transaction and return this for chaining", function () {
            const result = batchTransaction.addTransaction(mockTransaction1);

            expect(batchTransaction.transactions).to.contain(mockTransaction1);
    ...
    describe("getTransactionIds", function () {
        it("should return array of transaction IDs", function () {
            ...
            batchTransaction.setTransactions([tx, tx2]);

            const ids = batchTransaction.transactionIds;
    ...
    describe("fromBytes/toBytes", function () {
        it("should correctly convert to and from bytes", async function () {
            ...
            expect(restoredTx.transactions[0].alias.toString()).to.equal(
                privKey1.publicKey.toEvmAddress(),
            );
            expect(restoredTx.transactions[1].alias.toString()).to.equal(
                privKey2.publicKey.toEvmAddress(),
            );
            expect(restoredTx.transactions[0].key.toString()).to.equal(
                privKey1.publicKey.toString(),
            );
            expect(restoredTx.transactions[1].key.toString()).to.equal(
